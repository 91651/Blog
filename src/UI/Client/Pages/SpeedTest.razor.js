...
async function cale() {
    ...
    document.getElementById("speed").innerText = show((all_down_a - lsat_all_down) / (new Date().getTime() - lsat_date) * 1000, ['B/s', 'KB/s', 'MB/s', 'GB/s', 'TB/s', 'PB/s'], [0, 0, 1, 2, 2, 2]);
    document.getElementById("mbps").innerText = show((all_down_a - lsat_all_down) / (new Date().getTime() - lsat_date) * 8000, ['Bps', 'Kbps', 'Mbps', 'Gbps', 'Tbps', 'Pbps'], [0, 0, 0, 2, 2, 2]);
...
async function total() {
    ...
    document.getElementById("total").innerText = show((all_down_sum + all_down), ['B', 'KB', 'MB', 'GB', 'TB', 'PB'], [0, 0, 1, 2, 2, 2]);
...
function get_ipv4() {
    fetch('https://api-ipv4.ip.sb/geoip', { referrerPolicy: 'no-referrer' })
        ...
        .then(data => {
            var tag = document.getElementById("ipv4")
    ...
    setTimeout(get_ipv4, refresh_lay)
}
